...
const SourceRefreshContext = createContext<SourceRefreshContextValue | null>(null);
...
function readStoredSession(): StoredRefreshSession | null {
    try {
        ...
        if (typeof candidate.cursor !== 'string' || !Array.isArray(candidate.seen_job_ids)) return null;
        if (!candidate.seen_job_ids.every((value) => typeof value === 'string')) return null;
        return { cursor: candidate.cursor, seen_job_ids: candidate.seen_job_ids.slice(-MAX_SESSION_JOB_IDS) };
    } catch {
        return null;
    }
}
...
function noticeClasses(notice: RefreshNotice): string {
    if (notice.tone === 'danger') return 'border-red-500/40 bg-red-950/95 text-red-50';
    if (notice.tone === 'warning') return 'border-amber-500/40 bg-amber-950/95 text-amber-50';
    if (notice.tone === 'success') return 'border-emerald-500/40 bg-emerald-950/95 text-emerald-50';
...
export function SourceRefreshProvider({ enabled, children }: { enabled: boolean; children: ReactNode }) {
    ...
    const [latestActivityBatch, setLatestActivityBatch] = useState<RefreshActivityBatch | null>(null);
    ...
    const persistSession = useCallback(() => {
        ...
        sessionStorage.setItem(SESSION_KEY, JSON.stringify({ cursor: cursorRef.current, seen_job_ids: seen }));
    }, []);
    ...
    const applyStatus = useCallback((items: SourceRefreshStatusItem[]) => {
        const reconciled = items.map((item) => item.active_job && terminalJobIdsRef.current.has(item.active_job.job_id)
    ...
    const enqueueEvents = useCallback((events: SourceRefreshActivityEvent[]) => {
        ...
        aggregationTimerRef.current = window.setTimeout(() => {
            ...
            const totalCreated = grouped.reduce(
                (sum, event) => sum + (event.counts_authoritative ? event.created_count : 0),
                0,
            );
            setLatestActivityBatch({ id: batchId, events: grouped, total_created: totalCreated });
            addNotice(notificationForEvents(grouped, `activity-${batchId}-${grouped.at(-1)?.job_id ?? 'terminal'}`));
        }, NOTIFICATION_GROUP_MS);
    }, [addNotice]);
    ...
    useEffect(() => {
        ...
        void listSourceCatalog(controller.signal).then(({ data }) => {
            if (!controller.signal.aborted) setCatalog(data);
        }).catch(() => {
            if (!controller.signal.aborted) {
                setCatalog([]);
                setCatalogError('Source refresh capability is unavailable. Tender discovery is still available.');
            }
        }).finally(() => {
    ...
    useEffect(() => {
        ...
        const loadStatus = async (signal: AbortSignal): Promise<number | null> => {
            ...
            try {
                ...
                setStatusError(null);
                return data.filter((item) => item.active_job && !terminalJobIdsRef.current.has(item.active_job.job_id)).length;
            } catch {
                if (!signal.aborted && mountedRef.current) setStatusError('Refresh status unavailable.');
                return null;
            }
        };
        ...
        const drainActivity = async (signal: AbortSignal): Promise<{ events: SourceRefreshActivityEvent[]; more: boolean }> => {
            ...
            for (let page = 0; page < ACTIVITY_DRAIN_PAGE_LIMIT; page += 1) {
                ...
                try {
                    ...
                    const unique = activityEventsWithoutDuplicates(data.events, seenJobIdsRef.current);
                    unique.forEach((event) => terminalJobIdsRef.current.add(event.job_id));
        ...
        const pollCycle = async () => {
            if (!mountedRef.current || inFlightRef.current || document.visibilityState === 'hidden') return;
            ...
            try {
                ...
                const activeCount = statusItemsRef.current.filter((item) => item.active_job).length;
                if (previousActiveCount > 0 && activeCount === 0) gracePolls = COMPLETION_GRACE_POLLS;
                ...
                failureCount = activityFailed || reportedActiveCount === null ? failureCount + 1 : 0;
                schedule(more ? 0 : nextPollDelay(activeCount, gracePolls, failureCount));
            } finally {
        ...
        void (async () => {
            ...
            schedule(nextPollDelay(previousActiveCount, previousActiveCount > 0 ? COMPLETION_GRACE_POLLS : 0, failureCount));
        })();
        ...
        return () => {
            ...
            requestController?.abort();
            inFlightRef.current = false;
            if (aggregationTimerRef.current !== null) window.clearTimeout(aggregationTimerRef.current);
            aggregationTimerRef.current = null;
            aggregationQueueRef.current = [];
        };
    }, [applyStatus, enabled, enqueueEvents, persistSession]);

    const requestRefresh = useCallback(async (sourceSystem: string) => {
        const source = catalog.find((item) => item.source_system === sourceSystem);
        ...
        try {
            ...
            if ((normalizedStatus === 'queued' || normalizedStatus === 'running') && data.job_id) {
                const active = {
                    job_id: data.job_id,
                ...
                const existing = current.find((item) => item.source_system === sourceSystem);
                const next = existing
                    ? current.map((item) => item.source_system === sourceSystem ? { ...item, active_job: active } : item)
                    : current;
                applyStatus(next);
                addNotice({
                    id: `request-${data.job_id}`,
                    title: data.reused
                        ? `${data.display_name} refresh is already ${normalizedStatus}`
                        : normalizedStatus === 'queued'
                            ? `${data.display_name} refresh queued`
                            : `${data.display_name} refresh started`,
                    detail: null,
                    tone: 'info',
                    href: null,
                    action_label: null,
                ...
                addNotice({ id: `request-${sourceSystem}-${requestEpoch}`, title: `${data.display_name} is already up to date`, detail: null, tone: 'info', href: null, action_label: null });
            }
            wakePollRef.current?.();
        } catch {
            addNotice({ id: `request-error-${sourceSystem}-${requestEpoch}`, title: `${source.display_name} refresh could not be requested`, detail: 'Nothing was changed. Please try again.', tone: 'danger', href: null, action_label: null });
        } finally {
    ...
    const displayNameForSource = useCallback((sourceSystem: string) =>
        catalog.find((item) => item.source_system === sourceSystem)?.display_name ?? sourceSystem,
    [catalog]);
    ...
    }), [activeSources, catalog, catalogError, catalogLoading, displayNameForSource, latestActivityBatch, pendingSources, requestRefresh, retryCatalog, statusError, statusItems]);

    return <SourceRefreshContext.Provider value={value}>
        {children}
        <section aria-label="Refresh notifications" aria-live="polite" aria-relevant="additions" className="pointer-events-none fixed inset-x-3 bottom-3 z-[100] flex flex-col items-end gap-2 sm:left-auto sm:right-5 sm:w-[25rem]">
            {notices.map((notice) => <article key={notice.id} role={notice.tone === 'danger' ? 'alert' : 'status'} className={`pointer-events-auto w-full rounded-xl border p-4 shadow-2xl ${noticeClasses(notice)}`}>
                <div className="flex items-start gap-3">
                    <NoticeIcon notice={notice} />
                    <div className="min-w-0 flex-1"><p className="text-sm font-semibold">{notice.title}</p>{notice.detail ? <p className="mt-1 text-xs opacity-80">{notice.detail}</p> : null}</div>
                    <button type="button" onClick={() => dismissNotice(notice.id)} aria-label="Dismiss notification" className="rounded p-1 opacity-70 hover:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white"><X className="h-4 w-4" aria-hidden="true" /></button>
                </div>
                {notice.href && notice.action_label ? <Link href={notice.href} onClick={() => dismissNotice(notice.id)} className="mt-3 inline-flex rounded-lg border border-current/30 px-3 py-1.5 text-xs font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white">{notice.action_label}</Link> : null}
            </article>)}
        </section>
...
export function useSourceRefresh(): SourceRefreshContextValue {
    ...
    if (!context) throw new Error('useSourceRefresh must be used inside SourceRefreshProvider');
    return context;
}
...
export function GlobalRefreshIndicator() {
    ...
    if (!activeSources.length) return <span role="status" className="text-xs text-amber-300">Refresh status unavailable</span>;
    const single = activeSources[0];
    const label = activeSources.length === 1
        ? single.active_job?.status === 'queued'
            ? `${single.display_name} refresh queued…`
            : `Refreshing ${single.display_name}…`
        : `Refreshing ${activeSources.length} sources`;
    return <details className="relative">
        <summary role="status" aria-live="polite" className="flex cursor-pointer list-none items-center gap-2 rounded-lg border border-indigo-500/30 bg-indigo-500/10 px-3 py-2 text-xs font-semibold text-indigo-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400">
            <span className="h-2 w-2 rounded-full bg-indigo-400 motion-safe:animate-pulse" aria-hidden="true" />{label}
        </summary>
        <div className="absolute right-0 z-40 mt-2 w-64 rounded-xl border border-zinc-700 bg-zinc-950 p-3 shadow-2xl">
            <p className="mb-2 text-[10px] font-semibold uppercase tracking-wide text-zinc-500">Source refreshes</p>
            {activeSources.map((item) => <p key={item.source_system} className="py-1 text-xs text-zinc-200">{item.display_name} · {item.active_job?.status}</p>)}
        </div>
    </details>;
}
