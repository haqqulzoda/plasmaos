...
const event = (overrides = {}) => ({
    job_id: 'job-wb', source_system: 'world_bank', source_display_name: 'World Bank',
    status: 'completed', completed_at: '2026-09-01T10:00:00Z', fetched_count: 20,
    created_count: 12, updated_count: 3, unchanged_count: 5, skipped_count: 0,
    failed_count: 0, documents_discovered_count: 2, documents_queued_count: 2,
    counts_authoritative: true, fallback_used: false, degraded: false,
    terminal_reason: 'Refresh completed.', ...overrides,
});
...
test('exclusive-cursor defense deduplicates job ids without timestamp sorting', () => {
    ...
    const events = [event({ job_id: 'b' }), event({ job_id: 'a' }), event({ job_id: 'b' })];
    assert.deepEqual(activityEventsWithoutDuplicates(events, seen).map((item) => item.job_id), ['b', 'a']);
});

test('single completion copy preserves exact count, source, zero, and terminal truth', () => {
    assert.equal(notificationForEvents([event()], 'a').title, '12 new tenders from World Bank');
    assert.match(notificationForEvents([event({ created_count: 0 })], 'b').title, /no new tenders/);
    assert.match(notificationForEvents([event({ status: 'partial', created_count: 4 })], 'c').title, /with issues.*4 new tenders/);
    assert.match(notificationForEvents([event({ status: 'failed' })], 'd').title, /refresh failed/);
    assert.match(notificationForEvents([event({ status: 'source_unavailable' })], 'e').title, /could not be refreshed/);
    assert.match(notificationForEvents([event({ degraded: true })], 'f').title, /limited source coverage/);
});

test('multiple events aggregate counts while retaining mixed issue detail', () => {
    const notice = notificationForEvents([
        event(),
        event({ job_id: 'giz', source_system: 'giz', source_display_name: 'GIZ', created_count: 3, status: 'partial' }),
        event({ job_id: 'uzex', source_system: 'uzex', source_display_name: 'UzEx', created_count: 2 }),
    ], 'batch');
...
test('single-source action links to rolling source newness, never job membership', () => {
    ...
    assert.equal(href, '/dashboard/tenders?view=all&source=world_bank&new_only=true');
...
test('server reference defeats positive and negative browser wall-clock skew', () => {
    ...
    assert.ok(clock);
    assert.equal(adjustedServerNow(clock, 6_000), Date.parse('2026-09-01T10:00:05Z'));
    assert.equal(shouldShowNewBadge(true, '2026-09-01T10:00:10Z', clock, 6_000), true);
    ...
    assert.notEqual(browserWallClockPlusTwoHours, adjustedServerNow(clock, 6_000));
    assert.notEqual(browserWallClockMinusTwoHours, adjustedServerNow(clock, 6_000));
});
...
test('badge expires exactly at new_until and backend false never becomes true', () => {
    ...
    assert.equal(shouldShowNewBadge(true, '2026-09-01T10:00:10Z', clock, 10_049), true);
    assert.equal(shouldShowNewBadge(true, '2026-09-01T10:00:10Z', clock, 10_050), false);
    assert.equal(shouldShowNewBadge(false, '2027-09-01T10:00:10Z', clock, 50), false);
...
test('refresh POST remains explicit and is not owned by Explorer page effects', () => {
    ...
    assert.match(provider, /refresh queued|refresh started/);
    assert.doesNotMatch(provider, /Started.*reused/);
});

test('SR-3 contract files use explicit types without broad any', () => {
    for (const path of [
        'types/source-refresh.ts', 'lib/sourceRefresh.ts', 'lib/sourceRefreshPolicy.ts',
        'lib/tenderNewness.ts', 'components/source-refresh/SourceRefreshProvider.tsx',
        'components/source-refresh/SourceRefreshMenu.tsx', 'components/tenders/NewTenderBadge.tsx',
