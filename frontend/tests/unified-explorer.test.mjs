...
test("canonical unified request graph", () => {
  ...
  assert.match(page, /listExplorer\(\s*\{/);
...
test("filtered Explorer return state is shared with Tender Details", () => {
  assert.match(
    page,
    /writeExplorerReturnState\(\s*\{\s*explorerUrl: explorerHref/,
  );
  ...
  assert.doesNotMatch(
    page,
    /tender-explorer-restore|JSON\.stringify\(\{ tenderId: tender\.id, href/,
  );
});
...
test("filtered Explorer return state round-trips and rejects unsafe paths", () => {
  ...
  try {
    const state = {
      explorerUrl:
        "/dashboard/tenders?view=recommended&source=world_bank&q=water&page=2",
      tenderId: "tender-42",
      scrollY: 640,
      page: 2,
      createdAt: 1_000,
    ...
    values.set(
      ...
      JSON.stringify({
        ...state,
        explorerUrl: "https://example.invalid/dashboard/tenders",
      }),
    );
    assert.equal(readExplorerReturnState(), null);
...
test("truthful recommendation presentation", () => {
  ...
  for (const forbidden of [
    ...
    assert.doesNotMatch(recommendation, new RegExp(forbidden, "i"));
});
...
test("empty and profile-required states are truthful", () => {
  ...
  assert.doesNotMatch(
    page,
    /AI is analyzing your profile|Recommendations are generating|Check back soon/,
  );
});
