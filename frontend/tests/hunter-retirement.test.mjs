...
test("dead Hunter frontend implementation and types are removed", () => {
  ...
  assert.doesNotMatch(
    redirectPage,
    /Hunter Feed|AI-curated|No Recommendations Yet|scanning the market/,
  );
  assert.doesNotMatch(
    explorerPage,
    /\/hunter\/|HunterRecommendation|Hunter Feed/,
  );
});
...
test("unified Explorer remains canonical frontend authority", () => {
  assert.match(
    explorerClient,
    /api\.get<ExplorerResponse>\('\/explorer\/tenders'/,
  );
  assert.match(
    explorerClient,
...
test("legacy backend APIs remain compatibility-only and share mutation authority", () => {
  ...
  assert.match(
    backendHunter,
    /@router\.post\([\s\S]*recommendation_id[\s\S]*dismiss/,
  );
