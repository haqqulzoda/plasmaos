...
test('dead workspace and duplicate Tender components are removed', () => {
    for (const path of [
        ...
        assert.equal(existsSync(new URL(path, import.meta.url)), false, `${path} should be absent`);
    }
});

test('legacy lists and workspace are permanent read-only redirects', () => {
    assert.match(legacyBidList, /permanentRedirect\('\/dashboard\/bid-preparation'\)/);
    ...
    assert.doesNotMatch(`${legacyBidList}\n${legacyProposals}\n${legacyWorkspace}`, /api\.|fetch\(|post\(|put\(|patch\(|delete\(/i);
});

test('legacy bid detail validates only an owned Proposal ID', () => {
    assert.match(legacyBidDetail, /api\.get\(`\/proposals\/\$\{id\}`\)/);
    assert.match(legacyBidDetail, /router\.replace\(`\/dashboard\/bid-preparation\/\$\{id\}`\)/);
    assert.doesNotMatch(legacyBidDetail, /\/tenders\/|tender_id|api\.post|api\.put|api\.patch|api\.delete/);
});

test('Bid Preparation passive effects read persisted state and never synchronize', () => {
    assert.match(bidDetail, /api\.get<Proposal>\(`\/proposals\/\$\{resolvedParams\.proposalId\}`\)/);
    assert.match(bidDetail, /api\.get\('\/vault'\)/);
    assert.match(bidDetail, /api\.get<TenderDocument\[\]>\(`\/tenders\/\$\{tenderId\}\/documents`\)/);
    assert.doesNotMatch(bidDetail, /sync-docs|sync-status|pollTenderDocumentSync|TenderDocsSyncResponse|TenderSyncStatusResponse/);
});
...
test('Bid Preparation mutations remain explicit event handlers', () => {
    ...
    assert.doesNotMatch(bidDetail, /useEffect\([\s\S]{0,500}api\.(post|put|patch|delete)/);
});
...
test('canonical cross-surface links use the correct identifiers', () => {
    ...
    assert.match(tenderDetail, /`\/dashboard\/bid-preparation\/\$\{bidPreparation\.detail_route_id\}`/);
    assert.match(tenderDetail, /`\/dashboard\/tenders\/\$\{tender\.id\}\/compliance`/);
...
test('canonical navigation and runtime copy omit obsolete product terminology', () => {
    ...
    assert.doesNotMatch(runtime, /My Bids|Tender Workspace|Tender Draft|Draft Tender|Submit Bid|Submit Tender|Fully compliant|Ready %/);
});

test('dead Decision Snapshot and competitor client contracts are removed', () => {
    assert.doesNotMatch(tenderTypes, /TenderDecisionSnapshot|TenderCompetitor|competitorStatusLabel|competitorConfidence|competitorParticipation/);
...
test('all six canonical Tender Details anchors remain present', () => {
    for (const anchor of ['#pursuit', '#project-context', '#requirements-documents', '#compliance-readiness', '#contacts', '#bid-preparation']) {
