...
test("canonical dynamic route is Proposal-ID-only and passive", () => {
  ...
  assert.doesNotMatch(
    detail,
    /resolvedParams\.id|tender ID|tender_id: resolvedParams|api\.post<\{ id: string \}>\('\/proposals'/,
  );
  assert.doesNotMatch(
    detail,
    /sync-docs|sync-status|pollTenderDocumentSync|TenderSyncJob/,
  );
  assert.match(
    detail,
...
test("Compliance page no longer creates or resolves Proposal artifacts passively", () => {
  assert.doesNotMatch(
    compliance,
    /api\.post\('\/proposals'|\/proposals\/\$\{tenderId\}|proposal ID from/,
  );
});
