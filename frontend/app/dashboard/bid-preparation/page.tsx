        ...
        <div className="space-y-6">
            ...
            >
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold text-white">Bid Preparation</h1>
                        <p className="text-zinc-400 mt-1">Create and maintain preparation artifacts for your tenders</p>
                    </div>
                </div>
                <div className="flex items-center gap-2 px-4 py-2 bg-purple-500/10 border border-purple-500/20 rounded-full">
                    <span className="text-purple-400 text-sm font-medium">{proposals.length} Preparations</span>
                </div>
            </motion.div>
            ...
            {proposals.length === 0 ? (
                ...
                >
                    ...
                    <h3 className="text-xl font-semibold text-white mb-2">No bid preparations yet</h3>
                    <p className="text-zinc-400 mb-6">Choose Prepare Bid from a Tender or My Tenders.</p>
                    <Link
                                ...
                                <div className="flex items-start justify-between">
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-3 mb-2">
                                            ...
                                            <span className={`rounded-full border px-3 py-1 text-xs font-medium ${preparationStatusClasses(proposal.status)}`}>
                                                Preparation: {preparationStatusLabel(proposal.status)}
                                            </span>
                                            ...
                                            {proposal.engagement_status && (
                                                <span className="rounded-full border border-sky-500/30 bg-sky-500/10 px-3 py-1 text-xs font-medium text-sky-200">
                                                    Engagement: {proposal.engagement_status.charAt(0) + proposal.engagement_status.slice(1).toLowerCase()}
                                                </span>
                                            )}
                                        </div>
                                    ...
                                    <div className="flex items-center gap-2 ml-4">
                                        {!proposal.engagement_status && (
                                            <PrepareBidButton
                                                ...
                                                label="Continue Bid Preparation"
                                            />
                                        )}
